...
instruments. The `MVMR` R package facilitates estimation of causal effects using MVMR, as well as including a range of sensitivity analyses evaluating the underlying assumptions of the approach. The methods included in `MVMR` originate from Sanderson et al (2021), available [here](https://doi.org/10.1002/sim.9133).
...
The function `pleiotropy_mvmr()` can be used to test for heterogeneity, requiring the same arguments as the `strength_mvmr()`; `r_input` and `mvmrcov`.
...
#Example phenotypic correlations for illustration
mvmrcovmatrix<-matrix(c(1,-0.1,-0.05,-0.1,1,0.2,-0.05,0.2,1), nrow = 3, ncol = 3)
Xcovmat<-phenocov_mvmr(mvmrcovmatrix,F.data[,7:9])
res1 <- qhet_mvmr(F.data, Xcovmat, CI = TRUE, iterations = 100)
res1
```

It should be noted that as the number of exposure and instruments increases, estimation using `qhet_mvmr()` may prove difficult, owing to the substantial amount of computing power required.
