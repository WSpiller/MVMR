...
mvmr <- function(r_input, gencov, weights) {
  ...
  for (i in 1:exp.number) {
    ...
    regressors <- names(r_input)[
      -c(1, 2, 3, 4 + exp.number:length(names(r_input)))
    ]
  ...
  if (gencov != 0) {
    warning(
      "Validity statistics do not currently account for covariance between the effect of the genetic variants on each exposure"
    )
  }
